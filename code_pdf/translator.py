...
from code_pdf.cache import TranslationCache
...
class BaseTranslator:
    ...
    def set_envs(self, envs):
        # Tách khỏi self.__class__.envs
        # Không thể sử dụng self.envs = copy(self.__class__.envs)
        # bởi vì nếu set_envs được gọi hai lần, lần gọi thứ hai sẽ ghi đè lần gọi đầu tiên
    ...
    def add_cache_impact_parameters(self, k: str, v):
        """
        Thêm các tham số ảnh hưởng đến chất lượng dịch để phân biệt hiệu quả dịch thuật dưới các tham số khác nhau.
        :param k: khóa
        :param v: giá trị
        """
    ...
    def translate(self, text: str, ignore_cache: bool = False) -> str:
        """
        Dịch văn bản, các phần khác nên gọi phương thức này.
    ...
    def do_translate(self, text: str) -> str:
        """
        Thực hiện dịch văn bản, ghi đè phương thức này
        :param text: văn bản cần dịch
        :return: văn bản đã dịch
        """
    ...
    def prompt(
        ...
        return [
            {
                                                "role": "user",                "content": (                    "Bạn là một công cụ dịch máy chuyên nghiệp và đáng tin cậy. "                    "Chỉ xuất ra văn bản đã dịch, không bao gồm bất kỳ văn bản nào khác."                    "\n\n"                    f"Dịch văn bản nguồn markdown sau đây sang {self.lang_out}. "                    "Giữ nguyên ký hiệu công thức {v*}. "                    "Xuất bản dịch trực tiếp mà không có bất kỳ văn bản bổ sung nào."                    "\n\n"                    f"Văn bản nguồn: {text}"                    "\n\n"                    "Văn bản đã dịch:"                ),
            },
        ]
...
class QwenMtTranslator(OpenAITranslator):
    """
    Sử dụng mô hình Qwen-MT từ Aliyun, được thiết kế cho việc dịch thuật.
    Vì tiếng Trung phồn thể chưa được Aliyun hỗ trợ, nó sẽ được dịch sang tiếng Trung giản thể khi được chọn.
    Có các tham số đặc biệt trong tin nhắn gửi đến máy chủ.
    """
    ...
    def lang_mapping(input_lang: str) -> str:
        """
        Ánh xạ mã ngôn ngữ sang mã ngôn ngữ mà mô hình Aliyun Qwen-Mt hỗ trợ.
        Vì tất cả các mã ngôn ngữ hiện có được sử dụng trong gui.py đều có thể được ánh xạ,
        mã ngôn ngữ gốc sẽ không được kiểm tra.
        """
        langdict = {
            "zh": "Chinese",
            "zh-TW": "Chinese",
    ...
    def do_translate(self, text) -> str:
        """
        Mô hình Qwen-MT yêu cầu gửi translation_options đến máy chủ.
        Các domain là tùy chọn, nhưng được khuyến nghị. Chúng phải bằng tiếng Anh.
        """
        translation_options = {
