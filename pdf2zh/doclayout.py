...
class OnnxModel(DocLayoutModel):
    ...
    def extract_text_chunks(self, pdf_bytes):
        ...
        Returns:
            List of dictionaries containing text chunks with their bounding boxes for each page
            Format: {
                'pages': [
                    {
                        'page_number': 1,
                        ...
                        'height': 792,
                        'chunks': [
                            {'text': 'Example text', 'box': [x, y, width, height]},
                            ...
                        ]
                    },
                    ...
        ...
        # Process each page
        for page_idx, (page_mupdf, page_miner) in enumerate(zip(doc_mupdf, PDFPage.create_pages(doc))):
            ...
            # Process with layout model to get regions
            page_layout = self.predict(image, imgsz=int(pix.height / 32) * 32)[0]
            
            # Get text with pdfminer
            interpreter.process_page(page_miner)
            layout = device.get_result()
            
            # Filter text elements that correspond to text regions in our layout detection
            text_regions = []
            for i, d in enumerate(page_layout.boxes):
                # Text regions are those not in these classes
                if self._names[int(d.cls)] not in ["figure", "table", "isolate_formula", "formula_caption", "abandon"]:
            ...
            for element in layout:
                if isinstance(element, LTTextBox) or isinstance(element, LTText):
                    ...
                    # Find the region this text belongs to
                    text = element.get_text().strip()
                    if not text:
                        continue
                        
                    # Check which region this text belongs to
                    for region in text_regions:
                        r_x0, r_y0, r_x1, r_y1 = region['box']
                        
                        # Check if text box is mostly inside this region
                        # Use overlap threshold (50% of text box area should be within region)
                        text_box_width = x1 - x0
                        text_box_height = y1 - y0
                        text_box_area = text_box_width * text_box_height
                        ...
                        if i_x1 > i_x0 and i_y1 > i_y0:  # There is intersection
                            intersection_area = (i_x1 - i_x0) * (i_y1 - i_y0)
                            if intersection_area / text_box_area > 0.5:  # More than 50% of text is in this region
                                # Add text chunk to this region
                                region['chunks'].append({
                                    'text': text,
                                    'box': [int(x0), int(y0), int(x1 - x0), int(y1 - y0)]  # [x, y, width, height]
                                })
                                break
            
            # Flatten all chunks from all regions into the page data
            for region in text_regions:
                page_data['chunks'].extend(region['chunks'])
